...
namespace CooperativeProcurement.Analyzers.Analyzers
{
    ...
    public class MethodCommentAnalyzer : DiagnosticAnalyzer
    {
        ...
        private void AnalyzeMethodSyntactic(SyntaxNodeAnalysisContext context)
        {
            ...
            var isProtected = methodDeclaration.Modifiers.Any(SyntaxKind.ProtectedKeyword);
            var isInternal = methodDeclaration.Modifiers.Any(SyntaxKind.InternalKeyword);
        ...
        private bool HasXmlComment(MethodDeclarationSyntax method)
        {
            ...
            foreach (var trivia in leadingTrivia)
            {
                    ...
                    trivia.Kind() == SyntaxKind.MultiLineDocumentationCommentTrivia)
                {
                    ...
                    if (structure != null)
                    {
                        var documentation = structure as DocumentationCommentTriviaSyntax;
                        if (documentation != null)
                        {
                            // Поиск тега summary
                            var hasSummary = documentation.Content
